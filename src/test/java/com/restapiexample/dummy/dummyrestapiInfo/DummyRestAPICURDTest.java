...
public class DummyRestAPICURDTest extends TestBase {
    ...
    static String employeeName = "Kimi";
    @Steps
    ...
    public void test001() {
        ...
        createRecord.put("salary", "50000");
        createRecord.put("age", "30");
        createRecord.put("id", 34);
    ...
    @Title("3. Verify if 9th employee name is Colleen Hurst")
    @Test
    @Order(7)
    public void test007() {
        String employeeName = dummyRestAPISteps.readingAllEmployee().log().all().extract().path("data[9].employee_name");
        Assert.assertEquals(employeeName, "Colleen Hurst");
    }
    ...
    @Title("6. Verify 4th id salary is 433060")
    @Test
    @Order(10)
    public void test010() {
        List<?> employeeSalary = dummyRestAPISteps.readingAllEmployee().log().all().extract().path("data.findAll{it.id==4}.employee_salary");
        Assert.assertEquals(employeeSalary.get(0), 433060);
    }

    @Title("7. Verify 4th id age is 22")
    @Test
    @Order(11)
    public void test011() {
        List<?> employeeAge = dummyRestAPISteps.readingAllEmployee().log().all().extract().path("data.findAll{it.id==4}.employee_age");
        Assert.assertEquals(employeeAge.get(0), 22);
    }

    @Title("8. Verify 15th id name is Tatyana Fitzpatrick")
    @Test
    @Order(12)
    public void test012() {
        List<?> employeeName1 = dummyRestAPISteps.readingAllEmployee().log().all().extract().path("data.findAll{it.id==15}.employee_name");
        Assert.assertEquals(employeeName1.get(0), "Tatyana Fitzpatrick");
    }
